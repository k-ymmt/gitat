...
- [ ] 検索結果内での j/k ナビゲーション — 検索モード中にカーソルを上下移動してフィルタ結果を選択できるようにする
- [ ] 検索結果内でのプレビュー連動 — フィルタモードでカーソル移動時にプレビューパネルを更新する
- [ ] 検索結果から直接コミット詳細画面へ遷移 — Enter でフィルタ解除+カーソル移動だけでなく、直接 CommitDetail モードに入れるオプション
